...
mytree <- ape::read.tree('rabies_CpG/all_seqs_noout.nwk')
...
library("readxl")
mydata <- read.csv("N_CpG.csv")
# mydata = mydata[2:nrow(mydata),]

accessions = mydata$accessions#[1:(nrow(mydata)-1)]
mydata <- mydata[,c(6), drop = F]
#mydata <- mydata[1:(nrow(mydata)-1),]

# mydata <- mydata[,3:ncol(mydata)]
 mydata <- t(mydata)
 colnames(mydata) = accessions
# mydata = mydata[1:(nrow(mydata)-2),]
 mydatamat = data.matrix(mydata)
 class(mydatamat) <- "numeric"
